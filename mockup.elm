...
init = { time = 0, state = Start, noteTransparency = 0, aniTimeFade = 0, aniTimeExpand = 0, spiralTheta = 0, circleOfSixX = 0, circleOfSixY = 0 }
...
circleX x ani theta = if ani < 1.1 then 30
                    else ((a+b*(inverseTheta theta))*cos((speedOfSpiral ani)*theta))
                    --else if ani < 5 then ((10+1*(inverseAni ani))*cos(ani*0.1*ani))
                        --else ((10+1*(inverseAni ani))*cos(ani))

circleY y ani theta = if ani < 1.1 then 50
...
myShapes model = [-- BACKGROUND
           ...
           , myCircle
                ...
                |> move (circleX 0 model.aniTimeExpand model.spiralTheta, circleY 0 model.aniTimeExpand model.spiralTheta)       
           ...
           , myCircle
               ...
               |> move (circleX,(circleY))         --speed, % of the wheel
